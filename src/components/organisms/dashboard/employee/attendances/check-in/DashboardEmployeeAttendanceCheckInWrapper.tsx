...
import {
  checkInSchema,
  CheckInType,
} from "@/validators/attendances/checkin-attendance-validator";
import { useCheckInAttendance } from "@/http/attendances/checkin-attendance";
import { useRouter } from "next/navigation";

type PhotoMode = "camera" | "upload";

export default function DashboardEmployeeAttendanceCheckInWrapper() {
  ...
  const form = useForm<CheckInType>({
  ...
  const { mutate: checkIn, isPending } = useCheckInAttendance({
    onSuccess: () => {
      toast.success("Berhasil melalukan absen masuk!", {
        description: `Absensi masuk tercatat pada ${format(new Date(), "HH:mm")}`,
      });
      queryClient.invalidateQueries({ queryKey: ["get-attendance-today"] });
      router.push("/dashboard/attendances");
    },
    onError: (err) => {
      toast.error("Gagal melalukan absen masuk!", {
        description:
  ...
  const onSubmit = (values: CheckInType) => {
    checkIn({ body: values });
  };
